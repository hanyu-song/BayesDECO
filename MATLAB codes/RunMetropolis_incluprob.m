function[lpmodel_high,mlength,pInclu] = RunMetropolis_incluprob(start, y,x,...
...
%if nargin < 6 || isempty(PartIndex)
    %if strcmp(lower(alg_type), 'convBayes')
     %   PartIndex = '';
    %else error('Error. \n Must input the index.');
    %end
%end
...
%pInclu_all = repmat(0,postburn_count, p);
for iter = 1:num_iter
...
s0, kappa);
            ...
            %pInclu_all(iter, :) = pinclu_current;
        %else pInclu_all(iter, :) = 0;
        end
    end
end
pInclu = pInclu / postburn_count;
%name_str = sprintf('incluprob%dn%dp%dR2%dr.png',type_idx, n,p,R2*10);
